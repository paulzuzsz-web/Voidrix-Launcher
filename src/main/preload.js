...
contextBridge.exposeInMainWorld('voidrix', {
  ...
  library: {
    ...
    cancelInstall: (id) => call('library:cancelInstall', { id }),
    uninstall: (id) => call('library:uninstall', { id }),
    probeDownload: (url) => call('library:probeDownload', { url }),
    onDownloadProgress: (cb) => on('library:download-progress', cb),
